...
/*
 * ckptrd local functions
 */
void open_sock();
void close_sock();
void send_dsockmesg(const lam_cr_dsockmesg *);
void display_argv(int, char**);
char** remap(int *argc, char **argv);
...
main(int argc, char **argv)
{
  ...
  for(i=0;i<argc;i++)
  {
      ...
      if(length > 998)
          break;
      strncat(full_name, argv[i], 998 -length);
      strcat(full_name, " ");
  }
 ...
 */
  ...
  /* for ckptrd */
  char *temp = strstr(full_name, argv[argc - 1]);
  if(temp != NULL && (strstr(full_name, "-rmschema") == NULL))
      strncat(iden_name, temp, 99);
  else
      strcpy(iden_name, "");
  free(full_name);
 ...
 */
  ...
  /*
   * Sending dinfomesg to cktrd at MPISTART
   */
  lam_cr_dsockmesg dsockmesg;
  dsockmesg.dmesg_type = DINFO;

  lam_cr_dinfomesg *dinfomesg_ptr = &dsockmesg.dmesg.dinfomesg;

  dinfomesg_ptr->info_type = MPISTART;
  dinfomesg_ptr->mpirun_pid = getpid();
  dinfomesg_ptr->name = iden_name;
  dinfomesg_ptr->mpiworld_n = world_n;
  dinfomesg_ptr->mpi_gps = (struct _gps *) malloc(dinfomesg_ptr->mpiworld_n * sizeof(struct _gps));

  memcpy(dinfomesg_ptr->mpi_gps, mpiworld, dinfomesg_ptr->mpiworld_n * sizeof(struct _gps));

  dinfomesg_ptr->mpi_debug_info.nodeid = -1;
  dinfomesg_ptr->mpi_debug_info.mpi_pid = -1;
  dinfomesg_ptr->mpi_debug_info.killed_by_signal = -1;
  dinfomesg_ptr->mpi_debug_info.status = 0;

  open_sock();
  send_dsockmesg(&dsockmesg);
...
pwait(int4 *nwait, int *childstat)
{
  ...
  *childstat = 0;
  ...
  for (i = 0; *nwait > 0; ++i, --(*nwait)) {
    ...
    /*
     * Sending dinfomesg to cktrd at MPISTOP
     */
    lam_cr_dsockmesg dsockmesg;
    dsockmesg.dmesg_type = DINFO;
    ...
    if(status != 0)
	    dinfomesg_ptr->info_type = MPIERROR;
    else
	    dinfomesg_ptr->info_type = MPISTOP;
    dinfomesg_ptr->mpirun_pid = getpid();
    dinfomesg_ptr->name = iden_name;
    dinfomesg_ptr->mpiworld_n = world_n;
    dinfomesg_ptr->mpi_gps = (struct _gps *) malloc(dinfomesg_ptr->mpiworld_n * sizeof(struct _gps));
    ...
    dinfomesg_ptr->mpi_debug_info.nodeid = nodeid;
    dinfomesg_ptr->mpi_debug_info.mpi_pid = pid;
    dinfomesg_ptr->mpi_debug_info.killed_by_signal = killed_by_signal;
    dinfomesg_ptr->mpi_debug_info.status = status;
...
remap(int *argc, char **argv)
{
	...
	{
		if(strcmp(argv[i], "crtcp") == 0)
			ckptrd_status = 1;
		else if(strcmp(argv[i], "-ckptrd") == 0)
		{
			if(i == 1)
				(*argc)++;
			ckptrd_status = 1;
	...
	temp_argv = (char **) malloc((*argc) * (sizeof(char **)));

	for(i=0,j=0;i<temp_argc;i++)
	{
		if(strcmp(argv[i], "-ckptrd") == 0)
		{
			if(i == 1)
			temp_argv[j++] = strdup("-v");
			temp_argv[j++] = strdup("-ssi");
			temp_argv[j++] = strdup("cr");
			temp_argv[j++] = strdup("blcr");
			temp_argv[j++] = strdup("-ssi");
			temp_argv[j++] = strdup("rpi");
			temp_argv[j++] = strdup("crtcp");
		}
		else
		{
			temp_argv[j++] = strdup(argv[i]);
		}
	}
    temp_argv[j++] = NULL; // to counteract lam's bug
...
open_sock()
{
	...
	sockfd_ckptrd = socket (AF_INET, SOCK_STREAM, 0);
...
void
send_dsockmesg(const lam_cr_dsockmesg *dsockmesg)
{
	if(!ckptrd_status)
		return;
	...
	if(fl_verbose)
		printf("Sent dinfomesg with status %d\n", success);
