...
main(int argc, char *argv[])
{
  ...
  dsockmesg.dmesg_type = DBOOT;

  lam_cr_dbootmesg *dbootmesg_ptr = &dsockmesg.dmesg.dbootmesg;
  dbootmesg_ptr->node_count = nlamnet;
  dbootmesg_ptr->node_ips = (char **) malloc(nlamnet * sizeof(char *)); 

  for(i=0;i<nlamnet;i++)
      dbootmesg_ptr->node_ips[lamnet[i].lnd_nodeid] = strdup(inet_ntoa(lamnet[i].lnd_addr.sin_addr));
  ...
  for(i=0;i<nlamnet;i++)
  {
      ...
      inet_pton (AF_INET, dbootmesg_ptr->node_ips[i], &servaddr.sin_addr);
      connect (connfd, (struct sockaddr *) & servaddr, sizeof (servaddr));

      success = lam_ssi_ckptrd_write_dsockdmesg(connfd, &dsockmesg);
      if(fl_verbose || fl_debug)
          printf("Boradcasting boot mesg to %s with status %d\n", dbootmesg_ptr->node_ips[i], success);
  }
